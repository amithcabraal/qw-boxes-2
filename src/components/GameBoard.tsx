...
export const GameBoard: React.FC = () => {
  ...
  const findBestMove = useCallback(() => {
    ...
    // If no completing moves, avoid moves that give opponent a box
    ...
    for (let row = 0; row < GRID_SIZE; row++) {
      for (let col = 0; col < GRID_SIZE - 1; col++) {
        if (!lines.some(l => l.row === row && l.col === col && l.isHorizontal)) {
          ...
          if (wouldGiveBox) {
            riskyMoves.push({ row, col, isHorizontal: true });
          } else {
            safeMoves.push({ row, col, isHorizontal: true });
          }
        }
      }
    }
    ...
    for (let row = 0; row < GRID_SIZE - 1; row++) {
      for (let col = 0; col < GRID_SIZE; col++) {
        if (!lines.some(l => l.row === row && l.col === col && !l.isHorizontal)) {
          ...
          if (wouldGiveBox) {
            riskyMoves.push({ row, col, isHorizontal: false });
          } else {
            safeMoves.push({ row, col, isHorizontal: false });
          }
        }
      }
    }

    // Prefer safe moves, but make a risky move if necessary
  ...
  const makeMove = useCallback((row: number, col: number, isHorizontal: boolean) => {
    ...
    // Check for completed boxes immediately after setting lines
